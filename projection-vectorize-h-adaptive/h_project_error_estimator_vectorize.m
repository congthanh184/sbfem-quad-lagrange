...
function [localcoarse,uc,array,deltaerror,rhs65,error_array,rhs66,errest, ET, c_c_over_eig]= h_project_error_estimator_vectorize(p, supel1, pf, supel2)
...
for iel = 1:nel
    ...
    f1detmodij_mat = f1detmodij_mat(:) * ones(1, sdoff-2); % duplicate the column for sdof phi (sigma)
    f2detmodij_mat = ones(3, 1) * f2detmodij_col';  % duplicate so it has 3 entries for each sigma
    f2detmodij_mat = f2detmodij_mat(:) * ones(1, sdoff-2); % duplicate the column for sdof phi (sigma)
