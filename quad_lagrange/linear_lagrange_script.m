...
for idRow = 2:size(orimesh, 1) 
	...
	for idPt = 1:numel(pt)
		[N, NN, dN, dNN] = lagrangian2(pt(idPt), 1);
	...
	idEBlock = idRow*2; idSBlock = idEBlock - size(localE0, 1) + 1;
...
indexBounded = real(displacementModes) < 0;
phi = zeros(sdof, sdof);
q = zeros(sdof, sdof);

phi(:, 1:sum(indexBounded==1)) = eigVec(1:10, indexBounded);
q(:, 1:sum(indexBounded==1)) = eigVec(11:20, indexBounded) * G;	

globalStiffness = real(q/phi);
[globalStiffness,ff]=sbfeaplyc2(globalStiffness,supel.ff,supel.bcdof,supel.bcval);
...
local = [];
lambdaVal = displacementModes(indexBounded);
temp = repmat(eye(2), [5 1]);
phi(:, 9:10) = temp;
...
for idRow = 2:size(orimesh, 1)
	...
	for i = 1:numel(sinew)
		for j = 1:numel(nitanew)
			...
			[N, NN, dN, dNN] = lagrangian2(nitaj, 1);
			coordChildS = coordParent * NN';
			dCoordChildS = coordParent * dNN';
			Jacob = [coordChildS'; dCoordChildS'];
			detJ = det(Jacob);
