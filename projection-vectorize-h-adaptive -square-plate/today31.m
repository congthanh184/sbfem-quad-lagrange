...
idx_node1 = find(s(:,3)==8);
idx_node2 = find(s(:,4)==9);
...
%             dyds= dyds+local(iel).dhdnita(j)*(local(iel).gcoord(j,1)-elecenter(iel).coord(j,1)); % si=1
%         end
        dyds = local(iel).dhdnita * ( local(iel).gcoord(:, 1) - elecenter(iel).coord(:, 1));
