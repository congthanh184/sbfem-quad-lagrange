...
function [localcoarse,uc,array,deltaerror,rhs65,error_array,rhs66,errest, ET, c_c_over_eig]= h_project_error_estimator_vectorize2(p, supel1, pf, supel2)
...
for iel = 1:nel
    ...
    % scale det(J) to reduce error in calculation
    % scaleK_f12detmodij = 10 * max( abs( [f1detmodij_col; f2detmodij_col]));
    % scale_f1detmodij_mat = f1detmodij_mat / scaleK_f12detmodij;
    % scale_f2detmodij_mat = f2detmodij_mat / scaleK_f12detmodij;
    % [fcLarge, fcSmall] = separateMatrix(f_c_over_eig, 1.0);
    ...
    % e1t = XSum( XSum( real( f_c_over_eig .* (temp1 + temp2))));
    % e1t = XSum( XSum( real( f_c_over_eig .* temp1))) + XSum( XSum( real( f_c_over_eig .* temp2)));

    % modified calculation with matmtrx and scale det(J)
    e1t = sum(sum(f_c_over_eig .* temp1)) + sum(sum(f_c_over_eig .* temp2));
    % e1t = G * scaleK_f12detmodij * sum(sum(f_c_over_eig .* (temp1 + temp2)));
    % temp1 = transpose(fwt_array .* scale_f1detmodij_mat .* multiplyByD( matmtrx, one_fsigma_noD)) * one_fsigma_noD;
    % temp2 = transpose(fwt_array .* scale_f2detmodij_mat .* multiplyByD( matmtrx, two_fsigma_noD)) * two_fsigma_noD;
    % e1t = G * scaleK_f12detmodij * XSum( [ XSum( XSum( real( fcLarge .* ( temp1 + temp2)))), XSum( XSum( real(fcSmall .* ( temp1 + temp2))))] );
...
%                     nita = p64(int); 
%                     wt = w64(int);
%                     [N,shapelq,dhdnitalq,Nder]= lobatto(nita,pf(iel_fine1));%row-vectors
%                     [modij, invmodij]=sbfemodijcirlipa(nnel,supelf(1).centrecoord(iel_fine1).coord,shapelq,dhdnitalq,localfine(iel_fine1).xcrd,localfine(iel_fine1).ycrd);   
%                     [b1, b2, B1, B2]=sbfederiv2(nnel,shapelq,dhdnitalq,invmodij);
% 
%                     e1i = cf(i) * D * (-eigvalf(i) *B1 +B2) * localfine(iel_fine1).phi(:,i) ;
%                     e1j = cf(j) * D * (-eigvalf(j) *B1 +B2) * localfine(iel_fine1).phi(:,j) ;                    
%                     temp1 = wt * (1/(eigvalf(i)+eigvalf(j))) * transpose(e1i) * inv(D) * e1j * det(modij);
%                     temp1 = wt * (cf(i)*cf(j)/(eigvalf(i)+eigvalf(j))) * transpose((-eigvalf(i) *B1 +B2) * localfine(iel_fine1).phi(:,i)) * (G*matmtrx)' * ((-eigvalf(j) *B1 +B2) * localfine(iel_fine1).phi(:,j)) * det(modij);
%                     if isnan(temp1)
%                         temp1 = 0;
%                     end
%                     e1t1 = e1t1 + temp1;
%                     eijt1 = eijt1 + temp1;
%                     
%                     [N,shapelq,dhdnitalq,Nder]= lobatto(nita,pf(iel_fine2));%row-vectors
%                     [modij, invmodij]=sbfemodijcirlipa(nnel,supelf(1).centrecoord(iel_fine2).coord,shapelq,dhdnitalq,localfine(iel_fine2).xcrd,localfine(iel_fine2).ycrd);   
%                     [b1, b2, B1, B2]=sbfederiv2(nnel,shapelq,dhdnitalq,invmodij);
% 
%                     e1i = cf(i) * D * (-eigvalf(i) *B1 +B2) * localfine(iel_fine2).phi(:,i) ;
%                     e1j = cf(j) * D * (-eigvalf(j) *B1 +B2) * localfine(iel_fine2).phi(:,j) ;
%                     temp2 = wt * (1/(eigvalf(i)+eigvalf(j))) * transpose(e1i) * inv(D) * e1j * det(modij);
%                     temp2 = wt * (cf(i)*cf(j)/(eigvalf(i)+eigvalf(j))) * transpose((-eigvalf(i) *B1 +B2) * localfine(iel_fine2).phi(:,i)) * (G*matmtrx)' * ((-eigvalf(j) *B1 +B2) * localfine(iel_fine2).phi(:,j)) * det(modij);
%                     if isnan(temp2)
%                         temp2 = 0;
%                     end
%                     e1t2 = e1t2 + temp2;
...
%     end
    ...
    % delta = XSum(XSum( real(f_c_over_eig .* temp1))) + XSum(XSum( real(f_c_over_eig .* temp2)));
    ...
    % temp1 = transpose(fwt_array .* scale_f1detmodij_mat .* multiplyByD( matmtrx, one_error_si_noD)) * one_error_si_noD;
    % temp2 = transpose(fwt_array .* scale_f2detmodij_mat .* multiplyByD( matmtrx, two_error_si_noD)) * two_error_si_noD;
    % % delta = G * scaleK_f12detmodij * ( sum( sum( fcLarge .* ( temp1 + temp2))) + sum( sum( fcSmall .* ( temp1 + temp2))));
    % delta = G * scaleK_f12detmodij * XSum( [ XSum( XSum( real( fcLarge .* ( temp1 + temp2)))), XSum( XSum( real( fcSmall .* ( temp1 + temp2))))] );
    ...
    % e4t = XSum( XSum( real(c_c_over_eig .* temp)));

    % scaleK_cdetmodij = 10 * max( abs( full_cdetmodij_col(:)));
    % scale_cdetmodij_mat = cdetmodij_mat / scaleK_cdetmodij;

    % temp = transpose(cwt_array .* scale_cdetmodij_mat .* multiplyByD( matmtrx, full_csigma_noD)) * full_csigma_noD;
    % [fcLarge, fcSmall] = separateMatrix(c_c_over_eig, 1.0);    
    % % e4t = G * scaleK_cdetmodij * ( sum( sum( fcLarge .* temp)) + sum( sum( fcSmall .* temp)));
    % e4t = G * scaleK_cdetmodij * XSum( [ XSum( XSum( real(fcLarge .* temp))), XSum( XSum( real(fcSmall .* temp)))] );
    %==================================================
    ...
    % e2t = XSum( XSum( real(fc_c_over_eig .* temp1))) + XSum( XSum( real(fc_c_over_eig .* temp2)));

    % temp1 = transpose(fwt_array .* scale_f1detmodij_mat .* multiplyByD( matmtrx, one_fsigma_noD)) * half1_csigma_noD;
    % temp2 = transpose(fwt_array .* scale_f2detmodij_mat .* multiplyByD( matmtrx, two_fsigma_noD)) * half2_csigma_noD;
    % [fcLarge, fcSmall] = separateMatrix(fc_c_over_eig, 1.0);
    % e2t = G * scaleK_f12detmodij * ( sum( sum( fcLarge .* ( temp1 + temp2))) + sum( sum( fcSmall .* ( temp1 + temp2))));
    % e2t = G * scaleK_f12detmodij * ( XSum( XSum( real(fcLarge .* ( temp1 + temp2)))) + XSum( XSum( real( fcSmall .* ( temp1 + temp2)))));
    ...
    % e3t = XSum( XSum( real(cf_c_over_eig .* temp1))) + XSum( XSum( real(cf_c_over_eig .* temp2)));

    % temp1 = transpose(cwt_array .* multiplyByD( matmtrx, half1_csigma_noD)) * (one_fsigma_noD .* scale_f1detmodij_mat);
    % temp2 = transpose(cwt_array .* multiplyByD( matmtrx, half2_csigma_noD)) * (two_fsigma_noD .* scale_f2detmodij_mat);
    % [fcLarge, fcSmall] = separateMatrix(cf_c_over_eig, 1.0);
    % e3t = G * scaleK_f12detmodij * ( XSum( XSum( real( fcLarge .* ( temp1 + temp2)))) + XSum( XSum( real( fcSmall .* ( temp1 + temp2)))));
    %==================================================
    error_array(iel)= sqrt(real(-(e1t-e2t-e3t+e4t))); %LHS of eq 6.6
    errestop = errestop + real(-(e1t-e2t-e3t+e4t));
    errestbot = errestbot + real(-e1t);
